...
- Introduce a server-side fetch/cache library (e.g. TanStack Query) to replace the current manual OMDb-fetch-and-cache logic — see [Architecture & Stack](/engineering/architecture-and-stack.md) for the MVP approach (Zustand slice + manual fetch-and-cache).
