...
3. The user enters a search string (the movie title) into the search input, and the library is filtered live — without needing to click a search button. Filtering applies a **250ms debounce** (per [Stack & Tooling](/engineering/stack-and-tooling.md)) after the user stops typing, rather than re-filtering on every keystroke — the list is local/in-memory so a delay isn't required for performance, but it avoids re-rendering on every character for fast typers.
