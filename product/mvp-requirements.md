...
- A user can search their library, either across the entire library or scoped to a specific status (`watched` or `to watch`).
- A user can view metadata for a movie (both in their library and in OMDb search results), specifically:
  - Movie title
  - Release year
  - Runtime (in minutes)
- A user can provide their OMDb API key via the application.
...
- The UI will indicate to the user when:
  ...
  - Something has gone wrong (device offline, data persistence error, API error)
